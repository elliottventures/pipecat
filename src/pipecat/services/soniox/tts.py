...
class SonioxTTSService(WebsocketTTSService):
    ...
    async def on_audio_context_interrupted(self, context_id: str):
        ...
        if context_id and self._websocket and self._websocket.state is State.OPEN:
            cancel_msg = json.dumps({"stream_id": context_id, "cancel": True})
            try:
                await self._websocket.send(cancel_msg)
            except Exception as e:
                logger.warning(f"{self}: failed to send cancel for stream {context_id}: {e}")
        self._configured_contexts.discard(context_id)
        await super().on_audio_context_interrupted(context_id)

    async def on_audio_context_completed(self, context_id: str):
        """Forget per-stream state once an audio context has finished playing."""
        self._configured_contexts.discard(context_id)
    ...
    async def run_tts(self, text: str, context_id: str) -> AsyncGenerator[Frame | None, None]:
        ...
        try:
            ...
            try:
                if context_id not in self._configured_contexts:
